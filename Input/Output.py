...
# --- SPLUNK LOGGER BLOCK MIGRATION ---
# Commenting SplunkLogger block, replacing with Databricks logger initialization
...
def load_simpleui_data(simpleui_data_source: str) -> DataFrame:
    return (
        ...
        .select(
            F.col("printer_id").alias("printerId_from_simpleui"),
            F.col("app_deployed_uuid").alias("appDeployedUUID"),
        )
        .distinct()
        .dropna()
    )
