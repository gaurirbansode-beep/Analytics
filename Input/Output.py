...
def logging_wrapper(task, error_msg):
    def inner(func):
        def wrapper(*args, **kwargs):
            try:
                info(f"Wrapper starting {task}", data={"task": task, "state": STATE_STARTED})
                df = func(*args, **kwargs)
                info(f"Wrapper finished {task}", data={"task": task, "state": STATE_FINISHED})
                return df
            except AnalysisException as e:
                error(error_msg, data={"task": task, "dump": str(e), "state": STATE_ERROR})
...
def get_parquet_data(source: str, partition_string: str = "", retain_partition_columns: bool = "False") -> DataFrame:
...
def get_csv_data(source: str, separator: str = "|") -> DataFrame:
    return spark.read.format("csv").option("header", "true").option("sep", separator).load(source)
...
def get_redshift_data(redshift_constants: dict, create_session: bool) -> DataFrame:
    if create_session == True:
        sess = STSSession(arn=redshift_constants["CUMULUS_ARN"], session_name=redshift_constants["ARN_ROLE_SESSION_NAME"])
        secret = get_secret(redshift_constants["REDSHIFT_SECRET_ID"], session=sess.sts_session)
    else:
    ...
    df = spark.read.format("com.databricks.spark.redshift").option("url", jdbc_connection).option("query", redshift_constants["QUERY"]).option("tempdir", redshift_constants["TEMP_S3_DIR"]).option("forward_spark_s3_credentials", True).load()
    return df

def log_and_load_data(source_info: dict, log_data: dict) -> DataFrame:
    try:
        info(f"Starting {log_data['task']}", data={"task": log_data["task"], "state": STATE_STARTED})
        if source_info["format"] == "parquet":
            df = get_parquet_data(source_info["path"], source_info.get("partition_string"), source_info.get("retain_partition_columns"))
            ...
            df = get_redshift_data(source_info["constants"], source_info["create_session"])
        ...
        info(f"Finished {log_data['task']}", data={"task": log_data["task"], "state": STATE_FINISHED, "rows": df.count()})
        return df
    except AnalysisException as e:
        error(log_data["error_msg"], data={"task": log_data["task"], "dump": str(e), "state": STATE_ERROR})
        if str(e).startswith("Path does not exist:"):
            raise SourceEmptyException()
        else:
            raise e
